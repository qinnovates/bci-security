...
framework. **Validation is your responsibility.** All findings require
independent verification by qualified personnel. **Privacy of production
data is your responsibility.** Review all output before sharing externally.
```
...
3. **Strip org identifiers** — Replace company names, team names, and project names with generic labels unless the user explicitly opts to include them
...
5. **Strip credentials** — If any API keys, tokens, passwords, or secrets appear in scanned code, replace with `[REDACTED]` in the report. Never echo credentials back
...
If the user provides a `--include-context` flag or explicitly says to include identifying details, respect that choice but warn: "This report contains identifying details about your environment. Review before sharing externally."
...
independent verification by qualified personnel before acting on them.

**Privacy of production data is your responsibility.** This tool does not
transmit, store, or retain any data from your environment. However, reports
may reference patterns found in your code or configuration. Review all
generated output before sharing externally to ensure no sensitive
information (credentials, patient identifiers, proprietary architecture
details) is included.
